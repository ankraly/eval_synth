...
# - "split.txt" file indicating human/monolingual TTS gold system file split
# - "systems.txt" file indicating all non-default (experimental) systems
...
def write_pairs_ls(systems=[], split_gold=1):
    ...
    if split_gold==1:
        systems.extend([SPLIT,BASE])
    else:
        systems.extend([GOLDH,GOLDS,BASE])
    
    if SPLIT in systems:
        splat = open("split.txt","r")
        splot = (splat.readlines()).split("\t")
        for s in splot:
            if s[1] == "h":
                f = s+"_"+TGT+"_and_"+s+"_"+GOLDH
                t += "media_choice rating_instr audio 1.5 1 1 [["+f+"]] [0 1 2 3 4 5]\n"
            elif s[1] == "s":
                f = s+"_"+TGT+"_and_"+s+"_"+GOLDS
                t += "media_choice rating_instr audio 1.5 1 1 [["+f+"]] [0 1 2 3 4 5]\n"
...
def write_sent(systems):
    ...
    for tup in product(sents,systems):
        # tup[0] = sentence filename
        # tup[1] = system name
        if tup[0][:-4].endswith(tup[1]):    
...
def write_exp(name="time",split_gold=1,**kwargs):
    """
    writes LMEDS experiment provided user-specified parameters
    
    name: unique sequence name; if not provided, swapped for datetime string
    ...
    if ls != None:
        pairs = write_pairs_ls(ls,split_gold)
    elif num != None:
        if num <2 or (num == 2 and split_gold !=1):
            raise Exception ("You must use at least 3 systems, or 2 if you set split_gold=1")
        else:
            if split_gold==1:
            ...
            ls = sh[:num]
            # adding our default systems in and passing the list to write_pairs_ls:
            pairs = write_pairs_ls(ls, split_gold)    
    else:
        ...
        pairs = write_pairs_ls(split_gold)        
    exp.write(pairs+"\n")
